...
async function tmodelget(server: FastifyInstance){
    server.get('/', async (req) => {
        ...
        console.log(userid)
        let tmodelsid: any = await sequelize.query("SELECT * FROM user_text_model WHERE id_user='"+userid+"'", { type: QueryTypes.SELECT });
        let len = tmodelsid.length
        let tmodels: Array<any> = []
        let item;
        for(let i = 0;i<len;i += 1){
            item = await sequelize.query("SELECT * FROM text_models WHERE id='"+tmodelsid[i].id_text_models+"'", { type: QueryTypes.SELECT })
            tmodels.push(item[0])
        }
        return {tmodels}
    })
